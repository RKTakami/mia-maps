...
- **1b: per-shell origin snapping + per-shell `gsig`.** Mostly pure, testable without the renderer. **Do this BEFORE stage 2/3.** The focus is already free of the player (right-click pans, `R` recentres), and today ANY focus change rebuilds the whole grid — cascaded naively a drag would rebuild EVERY shell, which is worse than today. Snapping each shell's origin to its own cell grid means the 32blk shell shifts once per 32 blocks of pan, the 4blk once per 4, so a drag mostly rebuilds only the small inner shell.
- **2 step 1 ✅ DONE + owner-verified in game (`11f9495`)** — native context holds N shells: `begin`/`append`/`commit` on the worker, published atomically. **Shells are MERGED into one buffer, not drawn N times** — `render()` clears unconditionally, so N draws would re-enter the state discipline N times, and the depth-func fix (`a6fb204`), FBO detach (`380c6fc`) and `GlStateGuard` all live there. One VAO, one clear, one draw; unchanged. Merging required baking vertices to **world space** (the shader reconstructs `(uOrigin + aPos) * uCell` from per-mesh uniforms, and shells differ in cell size); uniforms are now identity. `has_content` also had to require real indices — `Some(vec![])` would have reported content with nothing to draw, re-creating the black-on-open bug (`cd7e65c`). **Caller still submits ONE shell, so behaviour is identical — that was the point.**
- **2 step 2 ✅ DONE + owner-verified in game (`3d4b79d`)** — real cascade rendering. **Area 2048/ULTRA: 8blk over 1536 (6.6M) + 16blk over 6144 (13.1M) = 19.7M cells, 49% of budget** vs flat 16blk/6144 at 13.1M. **2x the detail where you look, coverage unchanged, 1.5x the cost.** Native shells live in persistent SLOTS (`set_shell`/`commit(count)`) so only the shell that actually moved is re-meshed — commit truncates so a shrinking cascade leaves nothing stale.
  - **⚠ `planCascade` quarters the span per shell, NOT halves — do not "simplify" this.** Cost is `(span/cell)² × (vert/cell)`, and the Abyss vertical band is clamped far below the span (~1424 vs 6144), so it does NOT shrink with the shell. Halving both span and cell holds cells-per-axis constant while `vert/cell` DOUBLES inward, making inner shells cost MORE than outer ones — that put Area 2048 at **39.4M cells, 98% of budget, 3x today**. Quartering halves cells-per-axis inward. **The spec's "each shell costs about the same" only holds when vert ≈ span, which is not the real case.**
  - Two bugs avoided by re-reading existing constraints, worth not re-introducing: the plan must use **`GPU_MAX_LVL` (4), not `ORBIT_MAX_LVL` (5)** — Voxy stores nothing past 4 so an L5 shell samples EMPTY and the outer shell silently vanishes; and **whole-Abyss mode must null the shells**, or the submit draws stale cascade geometry instead of the column model.
- **▶ NEXT: (a) SEAMS** — 8blk/16blk boundary is untreated and visibly discontinuous; overlap by one coarse cell or emit skirts. **(b) L5 outer shells** via `LodUpsampler.mipInto` synthesis (MUST use the drawable-child predicate or `9b519db`'s pin-art holes return) — this is what unlocks **4blk** inner detail, currently capped at 8blk. **(c) the picking fix** below.
- **2 also owes the picking fix** — picking reads the CPU depth buffer while the screen shows the GPU mesh; `d65058b`'s focus-plane fallback papers over it. **Keep the fallback until real picking lands.**

**⚠⚠ BUILD TRAP — SECOND stale-native incident, now structurally guarded (`829661f`). Do not undo.** The 3D view died with `UnsatisfiedLinkError: nMeshBegin`: the jar's Java declared the new natives while the bundled dylib exported only the old one. Cause: **`copyMapNative` writes INTO `src/main/resources`, which is `processResources`' own input dir**, so Gradle fingerprinted it before the write landed and reported UP-TO-DATE, packaging the PREVIOUS dylib (`src/` 14:04:02 correct, `build/resources/` 13:58:18 stale) — **with a green build throughout**. `processResources` now never claims up-to-date and **fails the build** if the packaged dylib does not hash-match the fresh one. Same shape as the fork's `makeExcludedRocksDB` filter. **Standing rule: a green build proves NOTHING about natives — unpack the jar and check `nm -gU` / `lipo -info`.**
- Open wiring question: `OrbitScene` caps at `GPU_MAX_LVL = 4` but cascades want L5 outer shells (synthesized — MUST use `LodUpsampler.mipInto`'s drawable-child predicate or the pin-art holes of `9b519db` return).

**Also fixed 2026-07-27 (`d65058b`) — 3D right-click focus was silently discarded.** Owner reported it not recentring. The plumbing was always correct; the click never got that far. **`unprojectOffset` picks against the CPU raster's depth buffer while the screen shows the GPU mesh**, and they cover different ground (CPU cloud capped at `quality.maxPoints` at `lvl`/`extentXZ`; GPU at `gpuLvl`/`gpuExtentXZ`) — so clicking plainly-visible terrain could miss and return null. Now falls back to the focus plane, plus a "focus moved Nm — R to recentre" readout so a landed click is distinguishable from a dropped one. **Underlying mismatch remains: picking and display use DIFFERENT geometry. Fold the real fix (pick against what is actually drawn) into stage 2, rather than adding a second workaround.**
